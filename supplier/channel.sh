...
CORE_PEER_LOCALMSPID="SupplierMSP" CORE_PEER_TLS_ROOTCERT_FILE=${PWD}/peer/tls/ca.crt CORE_PEER_MSPCONFIGPATH=${PWD}/users/Admin@supplier.com/msp CORE_PEER_ADDRESS=localhost:9051 CORE_PEER_TLS_ENABLED=true FABRIC_CFG_PATH=${PWD}/../configtx peer channel join -b ../orderer/channel-artifacts/track.block
...
# update content
jq '.channel_group.groups.Application.groups.SupplierMSP.values += {"AnchorPeers":{"mod_policy": "Admins","value":{"anchor_peers": [{"host": "peer0.supplier.com","port": 9051}]},"version": "0"}}' config.json > modified_config.json
