...
set -xe
# chaincode
CORE_PEER_LOCALMSPID="SupplierMSP" CORE_PEER_TLS_ROOTCERT_FILE=${PWD}/peer/tls/ca.crt CORE_PEER_MSPCONFIGPATH=${PWD}/users/Admin@supplier.com/msp CORE_PEER_ADDRESS=localhost:9051 CORE_PEER_TLS_ENABLED=true FABRIC_CFG_PATH=${PWD}/../configtx peer lifecycle chaincode install ../basic.tar.gz

# chaincode check install
CORE_PEER_LOCALMSPID="SupplierMSP" CORE_PEER_TLS_ROOTCERT_FILE=${PWD}/peer/tls/ca.crt CORE_PEER_MSPCONFIGPATH=${PWD}/users/Admin@supplier.com/msp CORE_PEER_ADDRESS=localhost:9051 CORE_PEER_TLS_ENABLED=true FABRIC_CFG_PATH=${PWD}/../configtx peer lifecycle chaincode queryinstalled >&install.log
cat install.log
